  ...
  createImageWithDate: async (date, ctx) => {
    try {
      const res = await axios(
        `https://russianwarship.rip/api/v1/statistics/${date}`
      );
      ...
      // context.fillStyle = 'black' - Чорний фон для тексту / при потребі
      const imgText = `
    ...
    `;
      ...
      await ctx.replyWithPhoto({
        source: fs.createReadStream("resources/drawnImage.png"),
      });
    } catch (er) {
      console.log(er);
    }
